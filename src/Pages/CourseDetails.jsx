...
const renderContentPreview = (c) => {
  ...
  if (c.content_type === "document") {
    // Try inline preview with iframe (works best for pdf/doc/docx)
    return (
      <div className="mt-2">
        <iframe
    ...
    <div className="p-12 max-w-4xl bg-blue-200 mx-auto">
      ...
      {course ? (
        <>
          ...
          {course.thumbnail && (
            <img src={getFileUrl(course.thumbnail)} alt={course.title} className="w-full h-64 object-cover rounded mb-4" />
          )}

          {/* Upload/Edit form */}
          {user && user.role === "admin" && Number(course.instructor) === Number(user.id) && (
            <form onSubmit={handleSubmit} className="mb-6 border-t pt-4 space-y-2">
              <h2 className="text-lg font-semibold">{editingId ? "Edit Content" : "Upload Content"}</h2>
              ...
              <input type="number" value={order} onChange={(e) => setOrder(e.target.value)}
                placeholder="Order" className="w-full p-2 border rounded" />
              {!editingId && (
          ...
          <h2 className="text-xl font-semibold mt-4 mb-2">Contents</h2>
          {contents.length === 0 ? (
                ...
                <li key={c.id} className="p-4 border rounded bg-neutral-300 shadow-sm">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{c.content_type}</p>
